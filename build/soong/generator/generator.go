...
func init() {
	android.RegisterModuleType("orca_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = orcaExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = orcaExpandVariables(ctx, dep_file)
	...
	cmd := orcaExpandVariables(ctx, String(g.properties.Cmd))
