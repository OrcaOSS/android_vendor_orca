function __print_orca_functions_help() {
cat <<EOF
Additional Orca functions:
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/Orca-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.orca.device | grep -q "$ORCA_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $ORCA_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.orca.device | grep -q "$ORCA_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.orca.device | grep -q "$ORCA_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $ORCA_FIXUP_COMMON_OUT ]; then
